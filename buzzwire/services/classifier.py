...
CATEGORY_KEYWORDS = {
    ...
    "culture": ["culture", "viral", "trend", "creator", "internet", "meme", "popular", "gen z", "digital detox", "human-interest", "wholesome"],
}
...
def infer_emotional_triggers(text: str, categories: list[str], country_relevance: str) -> list[str]:
    ...
    if categories and set(categories).intersection({
        "engineering",
        "visual_explainer",
        "innovation",
        "entertainment",
...
def score_virality(
    ...
    if re.search(r"\b(first|largest|record|billion|million|quietly|secret|ban|race|impossible|hidden|simple|seconds|ever|history|most|biggest|longest|insane)\b", text):
        score += 0.8
